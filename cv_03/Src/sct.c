...
#define sct_nla(x) do { if (x) GPIOB->BSRR = (1 << 5); else GPIOB->BRR = (1 << 5); } while (0) 		// Latch pin ON/OFF
#define sct_sdi(x) do { if (x) GPIOB->BSRR = (1 << 4); else GPIOB->BRR = (1 << 4); } while (0)		// Data pin ON/OFF
#define sct_clk(x) do { if (x) GPIOB->BSRR = (1 << 3); else GPIOB->BRR = (1 << 3); } while (0)		// Clock pin ON/OFF
#define sct_noe(x) do { if (x) GPIOB->BSRR = (1 << 10); else GPIOB->BRR = (1 << 10); } while (0)	// Enable pin ON/OFF
...
void sct_led(uint32_t value)
{
	...
	{
		sct_sdi(value & 1); // Set the LSB
		value >>= 1;
		sct_clk(1);		// Time tick
		sct_clk(0);
	}

	sct_nla(1);			// Latch tick
	sct_nla(0);
}
